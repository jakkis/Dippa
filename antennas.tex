...
Efficiency can also be approximated with system's $S$-parameters. As radiated power is considered as power lost from the system, all $S$-parameters of an $N$-port system are required when determining the radiated power. For a lossless system, efficiency for port $i$ can be expressed as \cite{lehtovuori_phd}:
\begin{equation}
\label{eq:eff_aprx}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \begin{tikzpicture}
            ...
            \draw (-0.25,-1.75) node[label={below:...}] {};
...
This equation is valid for lossless systems, but it can be used as an approximation for lossy structures, similarly to (\ref{eq:eff_aprx}). ECC is used to evaluate the MIMO capability of an multiantenna system, and a coefficient below 0.5 is considered as good performance \cite{reduce_ecc,reduce_ecc2}.
